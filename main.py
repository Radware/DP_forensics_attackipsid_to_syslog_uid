...
def syslog_id_to_attackipsid(syslog_id):
    ...
    if syslog_id.startswith('FFFFFFFF-FFFF-FFFF-'):
        ...
        print(f'syslog ID {str(syslog_id)} , AttackIpsID {str(attackipsid)}')
...
def main():
    ...
    # remove duplicates
    id_list = list(dict.fromkeys(id_input))


    print(f'Attackipsid to Syslogid/SyslogID to Attackipsid v2.0 - by Egor Egorov')
    print(f'----------------------------------------------------------')
